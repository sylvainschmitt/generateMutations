...
rule iss_mutated:
    ...
    output:
        temp(expand("results/reads/mutated_N{N}_R{R}_AF{AF}_NR{NR}_R{strand}.fastq", strand=["1", "2"], allow_missing=True))
    params:
        N = "{N}",
        R = "{R}",
        AF = "{AF}",
    ...
    shell:
        ...
        "--o results/reads/mutated_N{params.N}_R{params.R}_AF{params.AF}_NR{params.NR} ; "
        "rm results/reads/mutated_N{params.N}_R{params.R}_AF{params.AF}_NR{params.NR}_abundance.txt"
