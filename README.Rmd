...
### [cp_files](https://github.com/sylvainschmitt/generateMutations/blob/main/rules/cp_files.smk)

* Tools: `cp`
* Parameters:
    * Sequence: `r yaml::read_yaml("config/config.yml")$sequence`
...
* Parameters:
    * Chromosome: `r yaml::read_yaml("config/config.yml")$chr`
...
* Parameters:
    * Number: `r yaml::read_yaml("config/config.yml")$n_mut`
    * Transition/Transversion ratio R (see below): `r yaml::read_yaml("config/config.yml")$R`
...
> ERROR:iss.util:Could not read temporary file: results/reads_N100_R2_AF0.6_NR50000/base.iss.tmp.Qrob_Chr01.0_R1.fastq

> ERROR:iss.util:You may have to remove temporary files manually

**This is not a true error as we can clean it manually but it stop `snakemake` for he moment.**
...
* Parameters:
    * Allele frequency: `r yaml::read_yaml("config/config.yml")$AF`
    * Number of reads: `r yaml::read_yaml("config/config.yml")$n_reads`
